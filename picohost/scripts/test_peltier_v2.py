...
def format_temp_status(data: dict) -> str:
    ...
    if data.get("status") == "update":
        ...
        return (
            f"[{timestamp}] Device initialized - {sensor_count} sensors found"
        )
...
def interactive_mode(peltier: PicoPeltier):
    ...
    print("Commands:")
    print(
        "  temp <value> [channel]  - Set target temperature (channel: 0=both, 1/2=individual)"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test Peltier temperature control"
    )
    parser.add_argument(
        "-p", "--port", help="Serial port (e.g., /dev/ttyACM0)"
    )
    parser.add_argument(
        "-b", "--baud", type=int, default=115200, help="Baud rate"
    )
    parser.add_argument(
        "--list", action="store_true", help="List available Pico devices"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Channel (0=both, 1/2=individual)",
    )
    parser.add_argument("--enable", action="store_true", help="Enable control")
    parser.add_argument(
        "--disable", action="store_true", help="Disable control"
    )
    parser.add_argument(
        "--monitor",
        action="store_true",
        help="Monitor only (no interactive mode)",
    )
