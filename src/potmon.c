...
static void pot_sensor_init(PotSensor *pot, uint gpio_pin, uint adc_channel)
{
    ...
    pot->r_total = POTMON_DEFAULT_R_TOTAL;
    pot->voltage = 0.0f;
    pot->resistance = 0.0f;
...
static void pot_sensor_read(PotSensor *pot)
{
    ...
    pot->voltage = v;
    float r = pot->r_total * pot->voltage / POTMON_VREF;
    pot->resistance = r;
...
void potmon_status(uint8_t app_id)
{
    send_json(7,
        ...
        KV_FLOAT, "pot0_resistance", pot0.resistance,
        KV_FLOAT, "pot1_voltage",    pot1.voltage,
        KV_FLOAT, "pot1_resistance", pot1.resistance
    );
}
