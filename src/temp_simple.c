...
#define THERMISTOR_ADC_SAMPLES 64
...
static float temp_sensor_read_voltage(const TempSensor *sensor) {
    ...
    for (int i = 0; i < THERMISTOR_ADC_SAMPLES; i++) {
        total += adc_read();
    }
...
bool temp_sensor_read(TempSensor *sensor) {
    ...
    if ((now - sensor->last_sample_time) < THERMISTOR_SAMPLE_INTERVAL_MS) {
        return false;
    }
